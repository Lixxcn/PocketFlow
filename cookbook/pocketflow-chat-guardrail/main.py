...
class GuardrailNode(Node):
    ...
    def exec(self, user_input):
        ...
        if not user_input or user_input.strip() == "":
            return (
                False,
                "Your query is empty. Please provide a travel-related question.",
            )

        if len(user_input.strip()) < 3:
            return (
                False,
                "Your query is too short. Please provide more details about your travel question.",
            )
...
```"""
        ...
        yaml_content = (
            response.split("```yaml")[1].split("```")[0].strip()
            if "```yaml" in response
            else response
        )
