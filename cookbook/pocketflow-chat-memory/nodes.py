...
class EmbedNode(Node):
    ...
    def exec(self, conversation):
        ...
        user_msg = next(
            (msg for msg in conversation if msg["role"] == "user"), {"content": ""}
        )
        assistant_msg = next(
            (msg for msg in conversation if msg["role"] == "assistant"), {"content": ""}
        )
    ...
    def post(self, shared, prep_res, exec_res):
        ...
        if "vector_index" not in shared:
            # 动态获取 embedding 维度
            embedding_dim = len(exec_res["embedding"])
            shared["vector_index"] = create_index(dimension=embedding_dim)
...
class RetrieveNode(Node):
    def prep(self, shared):
        ...
        latest_user_msg = next(
            (msg for msg in reversed(shared["messages"]) if msg["role"] == "user"),
            {"content": ""},
        )

        # Check if we have a vector index with items
        if (
            "vector_index" not in shared
            or "vector_items" not in shared
            or len(shared["vector_items"]) == 0
        ):
            return None
    ...
    def exec(self, inputs):
        ...
        return {"conversation": conversation, "distance": distances[0]}
